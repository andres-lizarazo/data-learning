...
marts, and RLS handles multi-tenant or regional data walls.`,
        ...
        {
          kind: "flashcards",
          title: "SQL — recall the essentials",
          cards: [
            { front: "Logical order of a SELECT", back: "FROM/JOIN → WHERE → GROUP BY → HAVING → SELECT → ORDER BY → LIMIT. (You write SELECT first, but it runs near the end.)" },
            { front: "WHERE vs HAVING", back: "WHERE filters **rows** before grouping; HAVING filters **groups** after aggregation (e.g. `HAVING COUNT(*) > 5`)." },
            { front: "INNER vs LEFT JOIN", back: "INNER keeps only matched rows. LEFT keeps every left row, filling unmatched right columns with NULL." },
            { front: "Window function vs GROUP BY", back: "GROUP BY collapses rows into one per group; a window function (`OVER (...)`) adds a per-row calculation while keeping every row." },
            { front: "COUNT(*) vs COUNT(col)", back: "`COUNT(*)` counts all rows; `COUNT(col)` counts rows where `col` is NOT NULL." },
