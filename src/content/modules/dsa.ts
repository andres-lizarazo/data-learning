...
print("min coins:", dp[amount])`,
    ...
    return dp[amount] if dp[amount] != INF else -1`,
        ...
        {
          kind: "flashcards",
          title: "Complexity & patterns — drill until reflex",
          cards: [
            { front: "Big-O of dict/set lookup", back: "Average **O(1)** (hash table). Worst case O(n) with pathological collisions, but treat it as O(1)." },
            { front: "Binary search complexity", back: "**O(log n)** — halves the search space each step. Requires **sorted** input." },
            { front: "Two-pointer / sliding window win", back: "Turns an O(n²) scan of pairs/subarrays into **O(n)** by moving pointers instead of restarting." },
            { front: "Hash map as a pattern", back: "Trade space for time: store seen values → O(1) lookup. Solves two-sum, duplicates, group-by in one pass." },
            { front: "BFS vs DFS", back: "BFS uses a **queue** → shortest path in unweighted graphs, level order. DFS uses a **stack/recursion** → explore deep, backtracking, topological sort." },
            { front: "When is it dynamic programming?", back: "Overlapping subproblems + optimal substructure. Cache subresults (memoize) or fill a dp table bottom-up (coin change, climbing stairs)." },
            { front: "Recursion's two required parts", back: "A **base case** (stops the recursion) and a **recursive step** on a strictly smaller input. Missing base case → stack overflow." },
          ],
        },
      ],
    },
